...
int main(int argc, char **argv) {
    ...
    on = (volatile unsigned short int*) (init_prumem() + ON_OFFSET);
    // Pointer to flag signifying encoder packets are ready to be collected
    encoder_ready = (volatile unsigned short int*) (init_prumem() + COUNTER_READY_OFFSET);
    // Pointer to data structure for encoder/counter packets
    encoder_packets = (volatile struct EncoderInfo*) (init_prumem() + COUNTER_OFFSET);
    // Pointer to flag signifying irig packets are ready to be collected
    irig_ready = (volatile unsigned short int *) (init_prumem() + IRIG_READY_OFFSET);
    // Pointer to data structure for IRIG packets
    irig_packets = (volatile struct IrigInfo *) (init_prumem() + IRIG_OFFSET);
    // Pointer to variable to identify that an error packet is ready to be writtento UDP
    error_ready = (volatile unsigned short int *) (init_prumem() + ERROR_READY_OFFSET);
    // Pointer to data structure for error packets
    error_packets = (volatile struct ErrorInfo *) (init_prumem() + ERROR_OFFSET);
